...
#' }
#' @export
testUnidirectionality = function(angles, gsets, Gdiffexp=NULL, minknown=2, minfound=2, maxknown=500, weight=T, weight_bw=100) {
  ...
  if (weight) {
    angles = circular::circular(angles, modulo = "2pi")
    dens = circular::density.circular(angles, bw=weight_bw)
    delta = dens$x[[2]] - dens$x[[1]]

    print(dens$y)

    weights = 1/(dens$y[floor(angles / delta)+1] *(2*pi))

    #weights = rescale(1/dens$y[floor(angles / delta)+1]**2)

    names(weights) = names(angles)
  } else {
    weights = setNames(rep(1, length(angles)), names(angles))
  }
  ...
  scores = do.call(rbind.data.frame, lapply(gsets, function(gset){
    ...
    weights_gset = weights[gset_filtered]

    angle = circularMean(angles_gset, weights_gset)
    #rayleigh_result = circular::rayleigh.test(angles_gset)

    list(pval=testRayleigh(angles_gset, weights_gset), angle=angle, n=length(gset_filtered))
  }))
...
#' @export
testRayleigh = function(angles, weights=rep(1, length(angles)), extran = 0) {
  n = sum(weights) + extran

  z = (n * (sqrt(sum(sin(angles) * weights)**2 + sum(cos(angles) * weights)**2)/n)**2)
  pval = exp(-z)
